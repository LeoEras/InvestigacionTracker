...
def loadNodes(dictionary, string):
    ...
    file_object.write("Id;Label;Description\n")

    if "App" in string or "Doc" in string:
        for items in objects:
            if string in items[7]:
                items[0] = depurado(items[0])
                if items[0] not in dictionary and items[8] == 1:
                    dictionary[items[0]] = id
                    file_object.write(str(id) + ";" + items[0] + ";" + items[6] + "\n")
                    id += 1
    else:
        for items in objects:
            if "App" in items[7]:                    
...
def loadEdges(dictionary, string):
    ...
    file_object.write("Source;Target;Label;Weight\n")
    dict_sum = {}
    pair = ""
    for index in range(0, len(objects)):
        next_item = index + 1
        if next_item < len(objects):
            if objects[index][0] in dictionary and objects[next_item][0] in dictionary_items:
                pair = str(dictionary[objects[index][0]]) + ";" + str(dictionary[objects[next_item][0]])
                if pair not in dict_sum:
                    dict_sum[pair] = 1
                else:
                    dict_sum[pair] += 1
    for item in dict_sum:
        file_object.write(item + ";" + str(dict_sum[item]) + ";" + str(dict_sum[item]) + "\n")
    file_object.close()
